...
function map(req, res) {
	peopleModel.people().then(function(people) {
		people = _.filter(people, function (person) { //filter out people without addresses
			return person.address;
		});
		people = _.unique(people, function (person) { //filter out duplicate addressees
			return person.address;
		});
		return peopleModel.geocode(people);
	}).then(function(people) {
		return peopleModel.kml(people);
	}).then(function(output) {
		res.kml(200, output, 'people');
	});
}
